...
function Select-Indices {
    ...
    if (-not $InstallImages -or $InstallImages.Count -eq 0) {
        Write-Verbose "No installed indices, so no index selection"
...
function Validate-ExtractISO {
    ...
    if ($existingJson -and ($existingJson.ISOPath -ne $ISO)) {
        ...
        Remove-Item $extractJson -Force -ErrorAction SilentlyContinue
...
function Invoke-ServiceWork {
    ...
    # Select anything that hasn't been
    #Select-Indices
    ...
    # Gather available packages, used to include both .msu and .cab, but .cab files don't install with DISM properly
    $ssuFiles = @(
        Get-ChildItem -Path $paths.KBsSSU -Include '*.msu' -Recurse -ErrorAction SilentlyContinue
    )
    $lcuFiles = @(
        Get-ChildItem -Path $paths.KBsOSCU -Include '*.msu' -Recurse -ErrorAction SilentlyContinue
    )
    ...
    function Service-Index {
        ...
        $imgNeeded = $true
        if ($imgExists -and $imgExists.Date) {
            try {
                $lastServiced = [datetime]::Parse($imgExists.Date)
                $imgNeeded    = $lastServiced -le $extractDate
            } catch {
                Write-Verbose "Service-Index: Failed to parse Date for $WimName index $($idx), will re-service"
                $imgNeeded = $true
            }
        }

        if (-not $imgNeeded) {
            Write-Host "    $WimName index $idx already serviced ($($imgExists.Date))"
            return
        }
        ...
        try {
            if ($InstallType) {
                ...
                # Wrapped inside a child try/catch block so a WinRE crash safely discards WinRE without breaking host environment
                try {
            ...
            Write-JsonFile -Path $imgJson -Data @{ Index = $idx; Name = $imgName; Date = (Get-Date -Format s) }
