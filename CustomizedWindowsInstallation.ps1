# Remember: variable names are CASE INSENSITIVE and assignments to script-level variables must use 'script:'
<#
.SYNOPSIS
...
.PARAMETER All
Shorthand for -KB -Export -Service -Final -Prep -Files -CreateISO
Default when no specific switch is provided

.PARAMETER Most
Same as -All without -KB
...
$LeadIn = { param($Label, $Value) '{0,-21}: {1}' -f $Label, $Value }
...
# Only output the HTML content for debugging these many times
$htmlContentOutput        = 1
$rawContentOutput         = 1
$detailsHtmlContentOutput = 100
...
$names = [ordered]@{
    ...
    InstallMissingDriversCmd = 'InstallMissingDrivers.cmd'
    InstallMissingDriversPs1 = 'InstallMissingDrivers.ps1'
...
function Show-Usage {
  ...
  Write-Host "  & $name [<Folder>] [-ISO <path>] [-DestISO <path>] [<Index Selections>] [<Work Selections>] [<More Options>]" -ForegroundColor Cyan
  ...
  Write-Host "  -Most       Shorthand for all work selections except -KB" -ForegroundColor Gray
  Write-Host ""
...
function Stream-FileCopy {
    ...
    # INITIALIZATION
    # -----------------------------
    $spaces = New-Object string ' ', $Indent
    ...
    $fmt           = "{0}Progress: {1,$($PercentWidth)}%  {2,$($maxBytesWidth):N0}/{3:N0} bytes"
    ...
    if (-not $NoTitle) { Write-Host ("{0}Copying {1}{2}" -f $spaces, (FolderRelName $SourcePath), $(if ($ShowSourceOnly) { "" } else { " to $(FolderRelName $DestinationPath)" })) }
    while (-not $success) {
        $sourceStream = $null
        $destStream   = $null
        ...
        try {
            ...
            while (($bytesRead = $sourceStream.Read($buffer, 0, $buffer.Length)) -gt 0) {
                ...
                if ($percentBucket -gt $lastReportedPercent) {
                    if ($doProgress) { Write-Host ($fmt -f $spaces, $percentBucket, $CopiedBytes, $TotalBytes) }
            ...
            if ($retries -gt 0) {
                Write-Warning ("{0}Copy failed: {1}" -f $spaces, $_.Exception.Message)
                Write-Warning ("{0}Retrying..." -f $spaces)
    ...
    if (-not $NoComplete) { Write-Host ("{0}Copy complete" -f $spaces) }
}
    ...
    try {
        ...
        foreach ($item in $allItems) {
            ...
            if ($item.PSIsContainer) {
                ...
                # Pass the $Indent configuration down to Stream-FileCopy
                Stream-FileCopy -SourcePath $item.FullName -DestinationPath $targetPath -CopiedBytes $CopiedBytes -TotalBytes $TotalBytes `
                                -ShowSourceOnly:$ShowSourceOnly -NoTitle:$NoTitle -NoProgress:$NoProgress -NoComplete -Indent $Indent
...
function Fetch-Date {
    ...
    try {
        ...
        # Property missing or empty
        if ($null -eq $meta -or [string]::IsNullOrWhiteSpace($meta.Date)) {
            return $NoTime
        }
        
        # Return the date
        return [datetime]::Parse($meta.Date)
    }
    catch {
        # Parsing crashed
        return $MissingTime
...
function Rebuild-Required {
    ...
    # Dependency file missing is fatal as you need to rebuild it
    $depDate = Fetch-Date $DependencyJsonPath
    if ($depDate -eq $MissingTime) {
        throw "$(FolderRelName $DependencyJsonPath) missing, you need to re-run with $SwitchName"   
    }
    if ($depDate -eq $NoTime) {
        throw "$(FolderRelName $DependencyJsonPath) corrupt, you need to re-run with $SwitchName"
    }

    # Rebuild it if the dependency is newer than the destination
    $destDate = Fetch-Date $DestJsonPath
    return ($depDate -gt $destDate)
}
...
function Run-Dism {
    ...
    try {
        ...
        while (-not $process.StandardOutput.EndOfStream) {
            ...
            if ($line.Trim()) { 
                Write-Verbose ($("{0}{1}" -f $spaces, $($line.Trim())))
            }
            ...
            if ($Capture) {
                ...
                if ($line -match '(\d+(?:\.\d+)?)%') {
                    ...
                    if ($percentBucket -gt $lastReportedPercent -and $percentBucket -le 100) {
                        Write-Host ($("{0}Progress: $percentBucket%" -f $spaces))
                        $lastReportedPercent = $percentBucket
                    }
                }
            }
        ...
        Write-Debug ("{0}$($process.StartInfo.FileName) $($process.StartInfo.Arguments) exited with code $(& $Hex $rc)" -f $spaces)
...
# Read WIM metadata from the given path
function Get-WimMetadata {
    ...
    # (WinOS, Version, Arch, Build, ServicePackBuild) in a single object, making two DISM calls:
    ...
    foreach ($line in $detailOutput) {
        ...
        if ($line -match '^\s*ServicePack Build\s*:\s*(\d+)') { $servicePackNumber = [int]$Matches[1] }
        if ($line -match '^\s*Architecture\s*:\s*(.+)')       { $archStr           = $Matches[1].Trim() }
    }
    Write-Debug "  build=$buildNumber servicePackBuild=$servicePackNumber arch=$archStr"
    ...
    return [PSCustomObject]@{
        ...
        Arch             = $detectedArch
        Build            = $buildNumber
        ServicePackBuild = $servicePackNumber
    }
}
...
function Validate-ExtractISO {
    [CmdletBinding()]
    param()

    # Try to read WIM metadata from the given path
    function Try-WimMetadata {
        ...
        if ($installWim -and $bootWim) {
            ...
            try {
                ...
                if (($metaInstall.Images.Count -gt 0) -and ($metaBoot.Images.Count -gt 0)) {
                    return [PSCustomObject]@{
                        WinOS            = $metaInstall.WinOS
                        Version          = $metaInstall.Version
                        Arch             = $metaInstall.Arch
                        Build            = $metaInstall.Build
                        ServicePackBuild = $metaInstall.ServicePackBuild
                        InstallImages    = @($metaInstall.Images | ForEach-Object { [PSCustomObject]@{ Index = [int]$_.Index; Name = $_.Name } })
                        BootImages       = @($metaBoot.Images    | ForEach-Object { [PSCustomObject]@{ Index = [int]$_.Index; Name = $_.Name } })
                        From             = $From
                    }
                } else {
                    Write-Verbose "Failed to read WIM metadata from '$From': $($metaInstall.Images.Count) install images and $($metaBoot.Images.Count) boot images"
                }
            } catch {
    ...
    # Let's first resolve the ISO path
    if (-not $ISO) {
        ...
        if ($isoFiles.Count -eq 0) {
            throw "No .iso file found in: $Folder`nPlace the Windows ISO there or use -ISO to specify its path"
        } elseif ($isoFiles.Count -gt 1) {
            throw ("Multiple .iso files found in: $Folder`n  {0}`nUse -ISO to specify which one to use" -f ($isoFiles.FullName -join "`n  "))
        } else {
            $script:ISO = $isoFiles[0].FullName
            Write-Host (&$LeadIn "Auto-discovered ISO" "$ISO")
        }
    }
    if ($ISO -and -not (Test-Path $ISO)) {
        throw "Source ISO ($ISO) not found, please correct and re-run the script"
    }

    # Final source ISO
    Write-Host (&$LeadIn "ISO path" "$ISO")

    # Final destination ISO
    if (-not $DestISO) {
        $script:DestISO = $ISO -replace '\.iso$', ($names.DefaultDestISO + '.iso')
        $script:AutoGeneratedDestISO = $true
        Write-Host (&$LeadIn "Auto-derived DestISO" "$DestISO")
    }
    
    # Resolve ISO and DestISO paths
    $script:ISO = Resolve-FullPath $ISO
    Write-Verbose (&$LeadIn "Resolved ISO path" "$ISO")
    $script:DestISO = Resolve-FullPath $DestISO
    Write-Verbose (&$LeadIn "Resolved DestISO path" "$DestISO")
    ...
    if ($DryRun) {
        ...
        Write-Host "[DryRun] Would copy tree -> $(FolderRelName $paths.SrcIsoRoot)"
        Write-Host "[DryRun] Would export indices to $(FolderRelName $paths.WimsIndices)"
        return
    }

    # Been down this path before, check if the ISO path has changed
    $extractJson  = $paths.ExtractJson
    $existingJson = Read-JsonFile -Path $extractJson
    $msg = "$(FolderRelName $extractJson) missing or corrupt"
    if ($existingJson -and ($existingJson.ISOPath -ne $ISO)) {
        $msg = "ISO path changed (was '$($existingJson.ISOPath)')"
        $existingJson = $null
    }
    if (-not $existingJson) {
        Write-Host "$msg, cleaning SrcIsoRoot and re-extracting..."
        Remove-Folder $paths.SrcIsoRoot
        Ensure-Folder $paths.SrcIsoRoot
    }

    # Read ISO / WIM metadata for WinOS / Version / Arch / Build / ServicePackBuild and index list
    ...
    if ($wimMeta -and ($wimMeta.ISOPath -eq $ISO)) {
        $relMetadataJson = $(FolderRelName $metadataJson)
        Write-Verbose "Loading metadata from $relMetadataJson"
        $wimMeta | Add-Member -NotePropertyName From -NotePropertyValue $relMetadataJson
        $writeMetadataJson = $false
    } else {
        # 2) Fall back to SrcIsoContent on disk
        $wimMeta = Try-WimMetadata -InstallWimPath $paths.InstallWimInSrc `
                                   -InstallEsdPath $paths.InstallEsdInSrc `
                                   -BootWimPath    $paths.BootWimInSrc `
                                   -From           "SrcIsoContent"
    }

    # 3) Mount the ISO, validate the files, and copy the tree to SrcIsoContent
    if (-not $wimMeta) {
        ...
        try {
            ...
            Write-Host "ISO mounted at: $metaDrive"

            # ---- Validate required files in ISO ----
            if (Report-Missing -Required @(
                "$metaDrive$($names.BootFileBIOS)",
                "$metaDrive$($names.BootFileUEFI)",
                "$metaDrive$($paths.BootWimInIso)"
            ) -AtLeastOne @(
            ...
            # Read WIM metadata from the mounted ISO
            $wimMeta = Try-WimMetadata -InstallWimPath "$metaDrive$($paths.InstallWimInIso)" `
                                       -InstallEsdPath "$metaDrive$($paths.InstallEsdInIso)" `
                                       -BootWimPath    "$metaDrive$($paths.BootWimInIso)" `
                                       -From           "Mounted ISO"

            if ($ShowIndices) {
                # We're only interested in the metadata, not the actual extraction
                $writeMetadataJson = $false
            } else {
                # Copy it and Stream-FolderCopy will give you a running commentary
                Stream-FolderCopy $metaDrive $paths.SrcIsoContent -ShowSourceOnly
        
                # Write the checkpoint file
                Write-JsonFile -Path $extractJson -Data @{ Date = (Get-Date -Format s); ISOPath = $ISO }
                Write-Host "ISO extraction complete ($extractJson written)"
            }
        } catch {
            # Clean up the partial SrcIsoContent so a re-run starts fresh
            Remove-Folder $paths.SrcIsoContent
            throw "ERROR during ISO extraction: $_, try again"
        } finally {
    ...
    # Apply WIM metadata to the script-scope variables
    $script:WinOS            = $wimMeta.WinOS
    $script:Version          = $wimMeta.Version
    $script:Arch             = $wimMeta.Arch
    $script:Build            = $wimMeta.Build
    $script:ServicePackBuild = $wimMeta.ServicePackBuild
    $script:InstallImages    = $wimMeta.InstallImages
    $script:BootImages       = $wimMeta.BootImages
    $script:MetaSrc          = $wimMeta.From

    # ShowIndices and we're done
    if ($ShowIndices) {
        if ($InstallImages.Count -eq 0) {
            throw "Cannot show indices: no metadata available, provide -ISO and run again"
        }
    ...
    # Write the metadata JSON file, if new
    if ($writeMetadataJson) {
        Write-JsonFile -Path $metadataJson -Data @{
            Date             = (Get-Date -Format s)
            ISOPath          = $ISO
            WinOS            = $wimMeta.WinOS
            Version          = $wimMeta.Version
            Arch             = $wimMeta.Arch
            Build            = $wimMeta.Build
            ServicePackBuild = $wimMeta.ServicePackBuild
            InstallImages    = $wimMeta.InstallImages
            BootImages       = $wimMeta.BootImages
        }
        Write-Host "WIM metadata saved ($($wimMeta.InstallImages.Count) install image(s), $($wimMeta.BootImages.Count) boot image(s))"
    }

    # Select anything that hasn't been, will set $InstallIndices
    Write-Debug "Select-Indices: Home=$SelectHome Pro=$SelectPro Indices='$Indices' TotalImages=$($InstallImages.Count)"
    ...
    if (-not $anyExplicit) {
        $script:InstallIndices = $InstallImages
        Write-Verbose "No explicit index selection, so selecting all $($InstallIndices.Count) indices: $($InstallIndices.Index -join ', ')"
        return
    }
    ...
    $script:InstallIndices = @($selected | Sort-Object Index -Unique)
    Write-Verbose "Index selection selected $($InstallIndices.Count) index/indices: $($InstallIndices.Index -join ', ')"
}
...
function Invoke-KBWork {
    ...
    function Invoke-CatalogRequest {
        ...
        try {
            ...
            Write-Debug "Content length = $($Response.Content.Length)"
            
            # Return pure text string content to bypass all DOM engines
    ...
    function Search-UpdateCatalogHtml {
        ...
        $htmlContent = Invoke-CatalogRequest -Uri $Uri
        if ([string]::IsNullOrEmpty($htmlContent)) {
            Write-Warning "No HTML string content returned from $Uri"
            return @()
        }
        
        # Only output the HTML content for debugging a finite number of times
        if ($script:htmlContentOutput -gt 0) {
            Write-Debug "HTML Content:`n$htmlContent"
            $script:htmlContentOutput--
        }

        Write-Verbose "Extracting update fields natively from HTML string via Regular Expressions"

        # Support single or double quotes around the table ID attribute
        if ($htmlContent -match '<table[^>]*id=[''"]ctl00_catalogBody_updateMatches[''"][^>]*>([\s\S]*?)</table>') {
            $tableHtml = $Matches[1]
        } else {
            Write-Verbose "No updates layout table container matched inside the page source."
            return @()
        }

        # Extract all rows from the table body content
        $rowPattern = '<tr[^>]*>([\s\S]*?)</tr>'
        $rowMatches = [regex]::Matches($tableHtml, $rowPattern)
        $extractedUpdates = @()

        foreach ($rowMatch in $rowMatches) {
            if ($MaxResults -gt 0 -and $extractedUpdates.Count -ge $MaxResults) {
                break
            }

            $rowHtml = $rowMatch.Groups[1].Value

            # Skip header rows or rows missing a GUID details trigger
            if ($rowHtml -notmatch 'goToDetails\([''"]([a-f0-9\-]{36})[''"]\)') {
                continue
            }
            $updateGuid = $Matches[1]

            # Extract table cell structures inside this specific row
            $cellMatches = [regex]::Matches($rowHtml, '<td[^>]*>([\s\S]*?)</td>')
            if ($cellMatches.Count -lt 7) {
                continue
            }

            # Helper to clean out HTML tags, space placeholders, and trailing whitespaces
            $GetCleanText = {
                param([string]$rawCell)
                $clean = [regex]::Replace($rawCell, '<[^>]+>', '').Replace('&nbsp;', ' ').Trim()
                return [System.Net.WebUtility]::HtmlDecode($clean)
            }

            # Extract properties
            $title          = &$GetCleanText $cellMatches[1].Groups[1].Value
            $products       = &$GetCleanText $cellMatches[2].Groups[1].Value
            $classification = &$GetCleanText $cellMatches[3].Groups[1].Value
            $lastUpdated    = &$GetCleanText $cellMatches[4].Groups[1].Value
            $version        = &$GetCleanText $cellMatches[5].Groups[1].Value
            
            # Get raw cell data directly containing human readable size and the exact trailing byte digits
            $rawSizeText    = &$GetCleanText $cellMatches[6].Groups[1].Value

            # --- CRITICAL FILTER: Skip if the update entry itself is a Preview release ---
            if ($title -imatch 'preview') {
                Write-Verbose "Skipping update search entry (Preview matched in title): $title"
                continue
            }

            # Extractions of the exact trailing bytes sequence (skips text & tracking spaces)
            $sizeInBytes = [long]0
            if ($rawSizeText -match '[\d\.,]+\s*(?:KB|MB|GB|Bytes|B)\s+(\d+)') {
                $sizeInBytes = [long]$Matches[1]
            }
            else {
                # Fallback block: strips out non-digits if layout spacing varies slightly
                $fallbackDigits = [regex]::Replace($rawSizeText, '\D', '')
                if (-not [string]::IsNullOrEmpty($fallbackDigits)) {
                    $sizeInBytes = [long]$fallbackDigits
                }
            }

            # Construct the PSCustomObject matching the script's pipeline expectation downstream
            $extractedUpdates += [PSCustomObject]@{
                Guid           = $updateGuid
                KBIndex        = $KBIndex
                Title          = $title
                Products       = $products
        ...
        Write-Verbose "Total IDs extracted: $($extractedUpdates.Count)"
        if ($extractedUpdates.Count -ne 0) {
            for ($i = 0; $i -lt $extractedUpdates.Count; $i++) {
                Write-Debug (&$Leadin $($i + 1) "")
                Write-Debug (&$Leadin "  GUID" $extractedUpdates[$i].Guid)
                Write-Debug (&$Leadin "  KBIndex" $extractedUpdates[$i].KBIndex)
                Write-Debug (&$Leadin "  Title" $extractedUpdates[$i].Title)
                Write-Debug (&$Leadin "  Products" $extractedUpdates[$i].Products)
                Write-Debug (&$Leadin "  Classification" $extractedUpdates[$i].Classification)
                Write-Debug (&$Leadin "  Last Updated" $extractedUpdates[$i].LastUpdated)
                Write-Debug (&$Leadin "  Version" $extractedUpdates[$i].Version)
                Write-Debug (&$Leadin "  Size" $extractedUpdates[$i].Size)
            }
        }
    ...
    function Get-UpdateLinks {
        ...
        # Only output the raw content for debugging a finite number of times
        if ($script:rawContentOutput -gt 0) {
            Write-Debug "Raw content:`n$($content)"
            $script:rawContentOutput--
        }
    ...
    function Get-UpdateDetails {
        ...
        $htmlContent = $detailsResponse.Content
        
        # Only output the HTML content for debugging a finite number of times
        if ($script:detailsHtmlContentOutput -gt 0) {
            Write-Debug "Details HTML Content:`n$htmlContent"
            $script:detailsHtmlContentOutput--
        }

        # Title Extraction via Regex
        $title = ""
        if ($htmlContent -match '<span[^>]*id=[''"]ScopedViewHandler_titleText[''"][^>]*>([\s\S]*?)</span>') {
            $title = [System.Net.WebUtility]::HtmlDecode($Matches[1].Trim())
        }
        Write-Verbose ("Title: {0}" -f $title)
    
        # --- SAFEGUARD: Explicitly filter preview updates discovered during deep inspection ---
        if ($title -imatch 'preview') {
            Write-Host ("{0} skipped (Preview update title found in details)" -f $Guid)
            return [System.Management.Automation.Internal.AutomationNull]::Value
        }
        ...
        # Capture everything following the target div open tag up until the next structural block container boundary.
        # This guarantees full multi-line coverage without missing subsequent links or choking on variable-length lookbehinds.
        if ($htmlContent -match '<div[^>]*id=[''"]supersededbyInfo[''"][^>]*>([\s\S]*?)(?=<div[^>]*id=|</body>|</html>)') {
            $supersededBlock = $Matches[1]
            
            # Extract all anchor link elements populated inside the block
            $linkMatches = [regex]::Matches($supersededBlock, '<a[^>]*>([\s\S]*?)</a>')
            foreach ($linkMatch in $linkMatches) {
                # Strip out inner structural markup formatting safely
                $rawLinkText = [regex]::Replace($linkMatch.Groups[1].Value, '<[^>]+>', '').Replace('&nbsp;', ' ').Trim()
                $supersededByKB = [System.Net.WebUtility]::HtmlDecode($rawLinkText)
                
                if ([string]::IsNullOrEmpty($supersededByKB)) {
                    continue
                }
    ...
    # Pull down each query and return the results
    $results = @()
...
function Invoke-Export {
    ...
    if ($DryRun) {
        ...
        Write-Host "[DryRun] Would export $($names.InstallWim) $($InstallIndices.Count) indices to $($paths.WimsIndices)"
    ...
    if (-not $extractMeta) {
        Write-Warning "$extractJson not found, -Clean and run again"
        return
    }
    $extractDate = Fetch-Date $extractMeta
    ...
    if (-not $installSrc -or -not $bootSrc) {
        throw "{0} image not found in $($paths.SourcesInSrc), -Clean and run again" -f 
        (@( if (-not $installSrc) { 'Install' } if (-not $bootSrc) { 'Boot' }) -join ' and ')
    }
    ...
    function Export-WimImage {
        ...
        foreach ($img in $Images) {
            ...
            $exportDest = Join-Path $paths.WimsIndices ("{0}_{1}"      -f $idx, $WimName)
            $exportJson = Join-Path $paths.WimsIndices ("{0}_{1}.json" -f $idx, $WimName)
            if (Rebuild-Required $extractJson $exportJson "-All -Clean") {
                Write-Host "    Exporting $WimName index $idx..."
                # Dism does not overwrite existing files, so we need to remove them first
                Remove-Item $exportDest -Force -ErrorAction SilentlyContinue
                Run-Dism @(
                    "/Export-Image",
                    "`"/SourceImageFile:$Src`"",
                    "`"/SourceIndex:$idx`"",
                    "`"/DestinationImageFile:$exportDest`"",
                    "/Compress:None",
                    "/CheckIntegrity"
                ) -Indent 4
                $rc = $LASTEXITCODE
                if ($rc -ne 0) {
                    ...
                    Write-Warning "    Try running this script again or using -Clean, skipping export for this index for now"
                    continue
                }
                Write-JsonFile -Path $exportJson -Data @{ Date = (Get-Date -Format s); Index = $idx; Name = $imgName }
                Write-Host "    $WimName index $idx exported"
            } else {
                Write-Host "    $WimName index $idx already exported"
            }
        }
    }

    # Export each selected image
    Export-WimImage -Images $InstallIndices -WimName $names.InstallWim -Src $installSrc
    Export-WimImage -Images $BootImages -WimName $names.BootWim -Src $bootSrc
...
function Invoke-ServiceWork {
    ...
    $extractJson = $paths.ExtractJson
    if (Report-Missing -Required @($extractJson)) {
        throw "See above for missing file details, need to -Clean first, and re-run"
    }
    $extractDate = Fetch-Date $extractJson
    ...
    function Get-ManifestPackages {
        ...
        if (Test-Path $manifestPath) {
            ...
            foreach ($item in $manifest) {
                $fullPath = Join-Path $ManifestDir $item.FileName
    ...
    function Add-Packages {
        ...
        # This copy is key as we need to get the file into Servicing so don't corrupt the original extracted file in case we need to retry servicing after a failure
        # If Dest is empty or the same as Src, we'll just service in-place
        $effectiveDest = $Dest
        if ([string]::IsNullOrWhiteSpace($Dest)) {
            $effectiveDest = $Src
            Write-Verbose "    Dest empty, using Src as Dest ('$effectiveDest')"
        } elseif ($Src -ne $Dest) {
            # Stream-FileCopy will tell us what it's doing
            Stream-FileCopy -SourcePath $Src -DestinationPath $Dest -Indent 4
            $effectiveDest = $Dest
        }
        ...
        # Track tracking parameter to verify if we are actively holding a mount open
        ...
        try {
            ...
            if ($Unmount) {
                ...
                Run-Dism @("/Unmount-Image", "`"/MountDir:$MountDir`"", "/Commit") -Indent 4
                if ($LASTEXITCODE -ne 0) {
    ...
    function Service-Index {
        ...
        if ($extractDate -gt $lastServiced) {
            $needsServicing = $true
        ...
        if ($needsServicing) {
            ...
            Write-Host "    $WimName index $idx already serviced ($lastServiced))"
            return
        }
    
        $mountDir = Join-Path $paths.WimsMounts   $idxName
        ...
        try {
            if ($InstallType) {
                ...
                $winrePathMount = Join-Path $paths.WimsMounts ("{0}_{1}_{2}" -f $idxName, $safeParent, $safeLeaf)
                ...
                # Boot WIM: copy extracted to serviced, mount, service with SSUs (may be empty), and dismount
                Add-Packages -IdxName $IdxName -MountDir $mountDir -Src $wimPath -Dest $srvPath -Mount $true -Unmount $true -PkgName "SSU" -Pkgs $ssuFiles
            }
    
            Write-JsonFile -Path $imgJson -Data @{ Date = (Get-Date -Format s); Index = $idx; Name = $imgName }
            Write-Host "    $WimName index $idx serviced"
        }
        catch {
            Write-Host "    ERROR servicing $WimName index $($idx). Rolling back changes." -ForegroundColor Red
            
            # Global clean recovery fallback mechanism
            if (Test-Path $mountDir) {
                Write-Host "    Emergency Unmounting and Discarding Parent Mount: $relMountDir" -ForegroundColor Yellow
            ...
            if (Test-Path $mountDir) {
                Write-Verbose "    Cleaning mount directory $relMountDir"
...
function Invoke-FinalAssembly {
    ...
    function Build-FinalImage {
        ...
        # Make sure all the serviced indices are newer than the final
        $needsRebuild = $false
        foreach ($img in $Images) {
            $idx = $img.Index
            ...
            if (Rebuild-Required $srvJson $finalJson "-Service") {
                Write-Host "$WimLabel index $idx ($($img.Name)) is newer than final, rebuilding"
                $needsRebuild = $true
                break
            }
        }
        ...
        if ($needsRebuild) {
            Write-Host "Rebuilding final $WimLabel..."
            Remove-Item $DestPath -Force -ErrorAction SilentlyContinue
            ...
            foreach ($img in $Images) {
                ...
                if ($LASTEXITCODE -ne 0) {
                    throw "ERROR exporting $WimLabel index $idx ($($img.Name)), run -Service again"
                }
            }

            Write-JsonFile -Path $finalJson -Data @{ Date = (Get-Date -Format s) }
        } else {
            Write-Host "Final $WimLabel is up-to-date"
        }
    }
...
function Invoke-Prep {
    ...
    Write-Host "Starting Prep workflow..."
    Write-Verbose "Invoke-Prep: SrcIsoContent='$($paths.SrcIsoContent)' DestIsoContent='$($paths.DestIsoContent)'"
    ...
    # 2. Finalized install.wim and boot.wim
    ...
    $finalInstallWimDate = Fetch-Date $finalInstallWimJson
    $finalBootWimDate    = Fetch-Date $finalBootWimJson
    ...
    if ($prepInstallWimDate -le $finalInstallWimDate) {
        ...
        Stream-FileCopy $paths.InstallWimInFinal $paths.InstallWimInDest -Indent 2
    ...
    if ($prepBootWimDate -le $finalBootWimDate) {
        # Stream-FileCopy will tell us what it is doing
        Stream-FileCopy $paths.BootWimInFinal $paths.BootWimInDest -Indent 2
        Write-JsonFile -Path $prepBootWimJson -Data @{ Date = (Get-Date -Format s) }
    } else {
        Write-Host "$($names.BootWim) already current ($(FolderRelName $prepBootWimJson): $prepBootWimDate)"
    }

    Write-Host "Prep workflow complete"
}
...
function Invoke-FilesWork {
    ...
    $silent = $DryRun -or $Clean

    if (-not $silent) { Write-Host "Starting Files workflow..." }
    ...
    $requiredFiles = $(
        ...
        $names.InstallMissingDriversCmd
        $names.InstallMissingDriversPs1
    )
    ...
    $transforms = @(
        ...
        @{
            ...
            Rules = @(
                ...
                @{ Pattern = '(?i)^(\s*set\s+"MISSINGCMD=%SRC%\\)[^"]*("\s*)$'; Replacement = $names.InstallMissingDriversCmd }
        ...
        @{
            File = $names.InstallMissingDriversPs1
            Rules = @(
                @{ Pattern = '(?i)^(\s*param\s*\(\s*\[string\]\s*\$Folder\s*=\s*'')[^''].*?(''\s*\)\s*)$'; Replacement = '.\' + $names.Drivers }
            )
        }
    )
    ...
    # Debug dump
    if (-not $silent -and ($DebugPreference -eq 'Continue')) {
        ...
        switch ($j.Action) {
            ...
            'SubFolderCopy' {
                ...
                elseif ($DryRun) { Write-Host "[DryRun] Would copy  : $(FolderRelName $j.From) to $(FolderRelName $j.To)" }
                                   ...
                                   Stream-FolderCopy -SourcePath $j.From -DestinationPath $j.To -NoComplete -Indent 2 }
            }
        }
    }
...
function Invoke-CreateISOWork {
    ...
    if (-not $DestISO) {
        Write-Warning "DestISO path is not set, use -DestISO or -ISO to provide"
        return
    }
    ...
    if ($AutoGeneratedDestISO -or -not $DestISOVolumeLabel) {
        # Get the metadata from the install.wim for the correct build and service pack build
        $metaInstall            = Get-WimMetadata -WimPath $paths.InstallWimInDest
        $buildServicePack       = "_{0}_{1}" -f $metaInstall.Build, $metaInstall.ServicePackBuild
        ...
        if (-not $DestISOVolumeLabel) {
            $DestISOVolumeLabel = "Win$($metaInstall.WinOS)_$($metaInstall.Version)_$($metaInstall.Arch)$($buildServicePack)"
        }
    }
