...
function Invoke-Export {
    ...
    function Export-WimImage {
        ...
        $needInstall  = (-not $existInstall) -or ([datetime]::Parse($existInstall.Date) -le $extractDate)

        if (-not $needInstall) {
            Write-Host "    $WimName index $idx already exported ($($existInstall.Date))"
        } else {
            ...
            Write-JsonFile -Path $installJson -Data @{ Date = (Get-Date -Format s); Index = $idx; Name = $imgName }
...
function Invoke-FinalAssembly {
    ...
    function Find-AnyOldServiced {
        ...
        $finalDate = Fetch-Date $finalJson
